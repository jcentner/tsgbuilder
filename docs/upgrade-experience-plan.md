...
- [x] Add `import urllib.request` and `import re` (both stdlib — no new dependency)
- [x] Add module-level cache variables:
  - `_latest_version: str | None = None`
  - `_update_url: str | None = None`
  - `_update_check_done: bool = False`
- [x] Add `_check_for_updates()` function:
...
- [x] Add `_is_newer(latest: str, current: str) -> bool` semver comparison helper:
...
- [x] In `main()`, spawn `_check_for_updates` in a daemon thread (alongside or after the telemetry thread):
  ```python
  threading.Thread(target=_check_for_updates, daemon=True).start()
  ```
- [x] In `api_about()`, add three new fields to the response dict:
...
- [x] At the end of `_check_for_updates()`, after caching values, if `_is_newer(latest, APP_VERSION)`:
...
- [x] Add `update_available` event to `docs/telemetry.md`:
...
- [x] Add entry to "File Reference" table if not already covering `web_app.py` (it does — no change needed)
...
- [x] In `loadAboutInfo()`, after building the version header div, add a conditional block:
...
- [x] Add `_isNewer(a, b)` JS helper function (mirrors backend logic):
...
- [x] In `DEFAULT_ENV_CONTENT` (line ~100 of `web_app.py`), add a commented-out entry:
  ```
  # Set to 0 to disable update checks
  # TSG_UPDATE_CHECK=0
  ```
- [ ] Document `TSG_UPDATE_CHECK=0` in the GETTING_STARTED.md telemetry section (it's adjacent to `TSG_TELEMETRY=0`) *(deferred to Deliverable 4)*

### 1E. Tests

- [x] Unit test `_is_newer()` with cases: equal versions, newer patch, newer minor, newer major, pre-release vs stable, malformed input — **17 tests in `test_version_check.py`**
- [x] Unit test `_check_for_updates()` with mocked `urllib.request.urlopen`:
  ...
  — **5 tests in `test_version_check.py`**
- [x] Unit test `api_about()` includes `latest_version`, `update_url`, `update_check_enabled` fields — **5 tests in `test_version_check.py`**
- [x] Unit test or integration test: `update_available` telemetry event fired when newer version detected — **covered in `test_success_sets_latest_version`**
...
- [x] In `save_agent_ids()`, add `"app_version": APP_VERSION` to the `data` dict:
  ```python
  data = {
...
- [x] In `updateSetupOverallStatus()`, after the agents-configured block:
...
- [x] Create `installer.iss` in repo root with Inno Setup directives:
  - `AppName=TSG Builder`
...
- [x] `[UninstallDelete]` section:
  - Clean up `_internal\` directory on uninstall
  - Do NOT delete `.env` (preserve user config)
- [x] `[Code]` section (optional):
  - `InitializeSetup` kills running `tsg-builder-windows.exe` before upgrade
...
- [x] Validate `installer.iss` syntax (this is a manual/CI step — Inno Setup's `ISCC` will fail on syntax errors)
...
- [x] Add `## Upgrading` section between **Quick Start** (ends ~line 82) and **Finding Your Configuration Values** (starts ~line 99):
  ```markdown
  ## Upgrading
...
- [x] In `docs/releasing.md`, add a note in the "Review and Publish" section (step 4, ~line 68):
  ```
...
- [x] Add `TSG_UPDATE_CHECK=0` to the Telemetry section of GETTING_STARTED.md alongside `TSG_TELEMETRY=0`
...
| Order | Deliverable | Dependencies | Est. Complexity | Status |
|-------|-------------|--------------|-----------------|--------|
| 1 | **2: Agent staleness** | None | Small — 2 files modified | **Done** |
| 2 | **1: Version check + banner** | None (independent) | Medium — backend thread + frontend | **Done** |
| 3 | **4: Upgrade docs** | After 1 & 2 (docs should reflect reality) | Small — docs only | **Done** |
| 4 | **3: Inno Setup installer** | After 4 (docs reference installer) | Medium — new file + CI changes | **Done** |
